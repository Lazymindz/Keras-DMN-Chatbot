...
working_dir = config.get('strings', 'working_dir')
train = config.get('strings', 'train')

batch_size= config.getint('ints', 'batch_size')
epochs= config.getint('ints', 'epochs')
...
class memoryNetwork(object):
    FILE_NAME = working_dir + 'model'
    VOCAB_FILE_NAME = working_dir + 'model_vocab'

    def __init__(self):

        if train != 'True' and (os.path.exists(memoryNetwork.FILE_NAME) and
